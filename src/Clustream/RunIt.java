...
import Clustream.addon.ClusterEvent;
import Clustream.addon.ClusterEventListener;
import Clustream.addon.Clustering;
import Clustream.addon.ClusteringStream;
import Clustream.addon.DataPoint;
import Clustream.addon.ClusterGenerator;
import Clustream.addon.General;
import Clustream.addon.MeasureCollection;
import Clustream.addon.RandomRBFGeneratorEvents;
import Clustream.addon.SSQ;

public class RunIt implements ClusterEventListener{
	...
	public RunIt(AbstractClusterer clusterer, ClusteringStream stream, MeasureCollection[] measures, int totalInstances){
	...
	public static void main(String[] args){
		...
		int measureCollectionType = 2;
		int amountInstances = 50000;
		String testfile = "D:\\Project\\ClustreamData\\test.csv";
	...
	public static void runBatch(RandomRBFGeneratorEvents stream, AbstractClusterer clusterer,
		...
		// run the batch job
		RunIt batch = new RunIt(clusterer, stream, measures, amountInstances);
		batch.run();
		...
		while(m_timestamp < totalInstances && stream.hasMoreInstances()){
			...
			if(counter <= 0){
				...
				if(useMicroGT && stream instanceof RandomRBFGeneratorEvents){
					gtClustering0 = ((RandomRBFGeneratorEvents)stream).getClustering();
				}
				...
				if(clusterer.implementsMicroClusterer()){
					if(clusterer instanceof ClusterGenerator
						...
						((ClusterGenerator)clusterer).setSourceClustering(((RandomRBFGeneratorEvents)stream).getClustering());
					}
					...
					if(clusterer.evaluateMicroClusteringOption.isSet()){
						...
						if(clustering0 == null && microC != null)
							clustering0 = Clustream.addon.KMeans.gaussianMeans(gtClustering0, microC);
					}
				}
